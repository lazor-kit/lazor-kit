...
export class LazorKitClient {
  ...
  > {
    ...
    // CSPRNG via Web Crypto. globalThis.crypto is native in modern React
    // Native runtimes (Hermes / JSC with crypto polyfills) and in browsers.
  ...
  private async resolveSecp256r1(walletPda: PublicKey, p: Secp256r1Params) {
    ...
    //   - slot:           getSlot() unless caller passed a slotOverride
    //   - publicKey+ctr:  one combined getAccountInfo via readAuthorityState
    //                     unless caller already has publicKeyBytes (then just
    //                     the counter is fetched).
    // Overrides short-circuit to Promise.resolve, so callers that pre-fetch
    // everything pay zero network cost here.
    const [slot, pubAndCounter] = await Promise.all([
      p.slotOverride != null
        ? Promise.resolve(p.slotOverride)
        : this.connection.getSlot().then((s) => BigInt(s)),
      p.publicKeyBytes
        ? readAuthorityCounter(this.connection, authorityPda).then((c) => ({
            publicKeyBytes: p.publicKeyBytes!,
            counter: c,
          }))
        : readAuthorityState(this.connection, authorityPda),
    ]);

    return {
      authorityPda,
      publicKeyBytes: pubAndCounter.publicKeyBytes,
      slot,
      counter: pubAndCounter.counter + 1,
    };
  }
